...
with open(f'OQuPy/results/processtensors/processtensorCF_a=0.1_T=10_dt={dt_finer}_ps=-{epsrel_smaller}_tcut={tcut}', 'rb') as f:
    process_tensorcf=dill.load(f)

# finer process tensor for dynamics 
with open(f'OQuPy/results/processtensors_10K/processtensor_a=0.1_T=10_dt={dt_finer}_ps=-{epsrel_smaller}_tcut={tcut}', 'rb') as f:
    process_tensor=dill.load(f)

# original control
with open(f'OQuPy/results/zero_control_10K/conv_checks/dt={dt_original}_ps=-{epsrel_original}_tcut={tcut}_x={b_x}_y=0.0_z=0.0/{tprot}ps', 'rb') as f:
...
folder="OQuPy/results/convergence_checks/a=0.1_T=10/"
